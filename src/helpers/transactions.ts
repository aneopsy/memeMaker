...
export async function awaitParsedConfirmedTransactions(
  txid: TransactionSignature,
  timeout: number,
  connection: Connection,
  commitment: Finality = "confirmed"
): Promise<ParsedConfirmedTransaction | null | void> {
  let done = false;
  return await new Promise(async (resolve, reject) => {
    ...
    do {
      console.log(".");
      try {
        const signatureStatuses: any =
          await connection.getParsedConfirmedTransactions([txid], commitment);
        const result = signatureStatuses && signatureStatuses[0];
        if (result) {
          done = true;
          resolve(result);
        }
